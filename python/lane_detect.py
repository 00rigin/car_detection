...
cap = cv2.VideoCapture('/home/whatacg/blackbox_data/test3.mov')
...
def compute_model_inverse(line, y_axis):
    # y_axis 는 darknet 에서 받아오는 차량의 중간 빨잔점의 y값
    #x = (1/m)(y-n)
    par = compute_model_parameter(line)
    #par[0] = m
    #par[1] = 1
    #par[2] = n
    res = (1/par[0])*(y_axis-par[2])
    #res 는 계산된 x값
    return res
...
def draw_fitline(img, result_l,result_r, color=(255,0,255), thickness = 10):
    ...
    #final = weighted_img(lane, img, 1,0.5)  darknet 때문에 빠짐. and return lane 으로 바꿈
    return lane
...
def detect_lanes_img(img):
    ...
    if len(L_lane) > 100:
        left_fit_line = smoothing(L_lane, 100)    
    if len(R_lane) > 100:
        right_fit_line = smoothing(R_lane, 100)
